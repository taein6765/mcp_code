# Cross-repository similarity analysis between MCP and Skills for cloning detection
...
BASE_DIR1 = os.path.abspath(os.path.join(SCRIPT_DIR, "../mcp_repo"))
BASE_DIR2 = os.path.abspath(os.path.join(SCRIPT_DIR, "../skills_repo"))
OUTPUT_FILE = "similarity_scores_cross.json"
...
for folder in tqdm(os.listdir(BASE_DIR1), desc="Indexing mcp repos"):
    path = os.path.join(BASE_DIR1, folder)
...
tqdm.write(f"Loaded {len(repos1)} mcp repos")

repos2 = []

for folder in tqdm(os.listdir(BASE_DIR2), desc="Indexing skills repos"):
    path = os.path.join(BASE_DIR2, folder)
    if not os.path.isdir(path):
        continue
...
tqdm.write(f"Loaded {len(repos2)} skills repos")

results = []

for r1 in tqdm(repos1, desc="Cross comparisons"):
    for r2 in repos2:
